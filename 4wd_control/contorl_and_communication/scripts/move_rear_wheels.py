...
def convert_vel_cmd(msg):
  ...
  print("x_vel:",x_vel, "y_vel:",y_vel, "w_vel: ", w_vel)
  # if abs(w_vel) ==0.0:
...
def main():
  ...
  #Initialising             
  rospy.init_node('hw_interface_node_rear_wheels')
  rospy.on_shutdown(turn_off)
  if not waitForArduino(): return 
  
  print('hw_interface node running_rear_wheels')
...
#   pub_encoder_ticks = rospy.Publisher("/Encoder_Ticks", ticks, queue_size = 1)
  ...
  while not rospy.is_shutdown():
    ...
    output_string = "<" + str(int(rear_left_w*max_speed)) + "," + str(int(rear_right_w*max_speed)) + ">"
